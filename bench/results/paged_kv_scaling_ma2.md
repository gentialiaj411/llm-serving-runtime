...
- Run UTC: `2026-06-16T02:08:24.415908+00:00`
- Model: `Qwen/Qwen2-1.5B-Instruct`
- GPU: `NVIDIA GeForce RTX 5070 Laptop GPU`
- Workload: `32` requests, variable prompt/output (seed `5070`), max_active `2`

## Peak memory

| Path | torch.cuda.max_memory_allocated (MB) | nvidia-smi peak used (MB) | tokens/sec |
|------|--------------------------------------|---------------------------|------------|
| Contiguous (`PHASE2_KV_BACKEND=reserved`) | 3234.0 | 4674.0 | 18.07 |
| Paged kernel (`PHASE2_KV_BACKEND=paged`) | 4076.3 | 5536.0 | 7.80 |

- Paged peak memory (PyTorch peak): **+26.04% higher** vs contiguous (`paged_vs_contiguous_torch_peak_delta_percent`; positive = paged uses more memory)
- Paged peak memory (nvidia-smi peak): **+18.44% higher** vs contiguous (`paged_vs_contiguous_nvidia_smi_peak_delta_percent`; positive = paged uses more memory)
