...
- Run UTC: `2026-06-16T02:00:24.786949+00:00`
- Model: `Qwen/Qwen2-1.5B-Instruct`
- GPU: `NVIDIA GeForce RTX 5070 Laptop GPU`
- Workload: `32` requests, variable prompt/output (seed `5070`), max_active `1`

## Peak memory

| Path | torch.cuda.max_memory_allocated (MB) | nvidia-smi peak used (MB) | tokens/sec |
|------|--------------------------------------|---------------------------|------------|
| Contiguous (`PHASE2_KV_BACKEND=reserved`) | 3213.2 | 4626.0 | 18.92 |
| Paged kernel (`PHASE2_KV_BACKEND=paged`) | 4062.6 | 5593.0 | 6.55 |

- Paged peak memory (PyTorch peak): **+26.43% higher** vs contiguous (`paged_vs_contiguous_torch_peak_delta_percent`; positive = paged uses more memory)
- Paged peak memory (nvidia-smi peak): **+20.90% higher** vs contiguous (`paged_vs_contiguous_nvidia_smi_peak_delta_percent`; positive = paged uses more memory)
